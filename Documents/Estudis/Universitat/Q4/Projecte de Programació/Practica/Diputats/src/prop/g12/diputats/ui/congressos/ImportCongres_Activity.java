...
public class ImportCongres_Activity extends JPanelFondo{
	...
	public ImportCongres_Activity() {
		...
		titol = new JLabel("Importar Congrés");
		...
		acceptar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (aux.exists()){
					try {
						ControladorPrincipal.getIO().llegirCongres(na);
					} catch (Exception e) {
						e.printStackTrace();
					}
					deleteView();
